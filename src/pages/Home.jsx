      ...
      <div className="relative min-h-[85vh] flex items-center justify-center text-center px-4 mb-12 overflow-hidden">
        <div className="relative z-10 max-w-5xl mx-auto animate-float">
          ...
          <div className="flex gap-4 justify-center">
             <button onClick={() => document.getElementById('products').scrollIntoView({behavior: 'smooth'})} className="px-6 py-3 md:px-10 md:py-5 rounded-full bg-slate-900 dark:bg-white text-white dark:text-slate-900 font-bold hover:bg-slate-800 dark:hover:bg-slate-200 transition duration-300 flex items-center gap-2 shadow-xl hover:scale-105 hover:shadow-2xl text-sm md:text-lg">
                Shop Collection <ArrowRight size={20}/>
             </button>
             <button onClick={() => navigate('/about')} className="px-6 py-3 md:px-10 md:py-5 rounded-full bg-white dark:bg-slate-800 border border-slate-200 dark:border-slate-700 text-slate-800 dark:text-white font-bold hover:bg-slate-50 dark:hover:bg-slate-700 transition duration-300 shadow-md hover:shadow-lg text-sm md:text-lg">
                Our Story
             </button>
          </div>
        </div>
      ...
      <div id="products" className="max-w-7xl mx-auto px-4 z-10 relative">
        ...
        <div className="flex flex-col gap-6 mb-10">
            ...
            <div className="relative w-full max-w-md mx-auto">
                <input 
                    ...
                    className="w-full py-4 pl-12 pr-4 rounded-full bg-white dark:bg-slate-800 border border-slate-200 dark:border-slate-700 shadow-sm focus:outline-none focus:ring-2 focus:ring-violet-200 dark:focus:ring-violet-900 text-slate-700 dark:text-white font-bold transition placeholder:text-slate-400 dark:placeholder:text-slate-500"
                    value={searchTerm}
            ...
            <div className="flex flex-wrap gap-2 justify-center">
                <button 
                    ...
                    className={`px-4 py-2 rounded-full text-sm font-bold transition ${selectedCategory === 'All' ? 'bg-slate-900 dark:bg-white text-white dark:text-slate-900 shadow-lg' : 'bg-white dark:bg-slate-800 text-slate-600 dark:text-slate-300 hover:bg-slate-50 dark:hover:bg-slate-700 border border-slate-200 dark:border-slate-700'}`}
                >
                    All
                </button>
                ...
                {activeCategories.map(c => (
                    <button 
                        ...
                        className={`px-4 py-2 rounded-full text-sm font-bold transition ${selectedCategory === c ? 'bg-slate-900 dark:bg-white text-white dark:text-slate-900 shadow-lg' : 'bg-white dark:bg-slate-800 text-slate-600 dark:text-slate-300 hover:bg-slate-50 dark:hover:bg-slate-700 border border-slate-200 dark:border-slate-700'}`}
                    >
                        {c}
                    </button>
                ))}
        ...
        <div className="grid grid-cols-2 lg:grid-cols-4 gap-3 md:gap-8">
          {filteredProducts.map(p => {
            ...
            return (
            <div key={p.id} className="group bg-white/70 dark:bg-slate-800/70 backdrop-blur-md rounded-2xl md:rounded-[2rem] overflow-hidden border border-white/60 dark:border-slate-700 shadow-sm hover:shadow-2xl transition duration-500 flex flex-col relative">
              ...
              <div className="relative w-full aspect-[3/4] overflow-hidden cursor-pointer bg-slate-50 dark:bg-slate-900" onClick={() => navigate(`/product/${p.id}`)}>
                ...
                {/* Wishlist Button */}
                <button onClick={(e) => { e.stopPropagation(); toggleWishlist(p); }} className="absolute top-3 right-3 p-2 rounded-full bg-white/80 dark:bg-slate-900/80 hover:bg-white dark:hover:bg-slate-900 shadow-sm transition z-10">
