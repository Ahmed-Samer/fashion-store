...
// دي القائمة "الماستر" (الترتيب اللي إنت عايزه)
...
const Home = ({ products, addToCart, wishlist, toggleWishlist }) => {
  ...
  // 1. (الجديد) استخراج الأقسام النشطة فقط
  // بنستخدم useMemo عشان الحسبة دي متتعملش غير لما المنتجات تتغير
  const activeCategories = useMemo(() => {
    // هاتلي بس الأقسام اللي ليها منتج واحد على الأقل موجود في قائمة products
    return MASTER_CATEGORIES.filter(category => 
      products.some(p => p.category === category)
    );
  }, [products]);

  // 2. فلترة المنتجات للعرض
        ...
        <div className="flex flex-col gap-6 mb-10">
            ...
            <div className="relative w-full max-w-md mx-auto">
                <input 
                    ...
                    className="w-full py-4 pl-12 pr-4 rounded-full bg-white border border-slate-200 shadow-sm focus:outline-none focus:ring-2 focus:ring-violet-200 text-slate-700 font-bold transition"
                    value={searchTerm}
            ...
            <div className="flex flex-wrap gap-2 justify-center">
                <button 
                    ...
                    className={`px-4 py-2 rounded-full text-sm font-bold transition ${selectedCategory === 'All' ? 'bg-slate-900 text-white shadow-lg' : 'bg-white text-slate-600 hover:bg-slate-50 border border-slate-200'}`}
                >
                    All
                </button>
                
                {/* (التغيير هنا) بنعرض activeCategories بدلاً من القائمة الثابتة */}
                {activeCategories.map(c => (
                    <button 
                        key={c} 
                        onClick={() => setSelectedCategory(c)} 
                        className={`px-4 py-2 rounded-full text-sm font-bold transition ${selectedCategory === c ? 'bg-slate-900 text-white shadow-lg' : 'bg-white text-slate-600 hover:bg-slate-50 border border-slate-200'}`}
                    >
                        {c}
                    </button>
                ))}
