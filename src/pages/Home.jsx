...
const containerVariants = {
  ...
  show: {
    opacity: 1,
    transition: {
      staggerChildren: 0.05 // قللنا الوقت عشان يبقى أسرع
    }
  }
};

const itemVariants = {
  hidden: { opacity: 0, y: 10 }, // قللنا المسافة لـ 10 بيكسل بس
  show: { opacity: 1, y: 0, transition: { duration: 0.3 } } // لغينا الـ spring التقيل
};
        ...
        <div className="relative z-10 max-w-5xl mx-auto animate-float">
          ...
          <div className="flex gap-4 justify-center">
             <motion.button 
                ...
                className="px-6 py-3 md:px-10 md:py-5 rounded-full bg-slate-900 dark:bg-white text-white dark:text-slate-900 font-bold transition-colors duration-300 flex items-center gap-2 shadow-xl hover:shadow-2xl text-sm md:text-lg"
             >
             ...
             <motion.button 
                ...
                className="px-6 py-3 md:px-10 md:py-5 rounded-full bg-white dark:bg-slate-800 border border-slate-200 dark:border-slate-700 text-slate-800 dark:text-white font-bold transition-colors duration-300 shadow-md hover:shadow-lg text-sm md:text-lg"
             >
                Our Story
             </motion.button>
          </div>
          ...
          {filteredProducts.map(p => {
            ...
            >
              ...
              <div className="relative w-full aspect-[3/4] overflow-hidden cursor-pointer bg-slate-50 dark:bg-slate-900" onClick={() => navigate(`/product/${p.id}`)}>
                {/* 🚀 تعديل الأداء: شيلنا layoutId وضفنا loading="lazy" */}
                <motion.img 
                    whileHover={{ scale: 1.05 }}
                    ...
                    loading="lazy"
                    decoding="async"
                    className={`w-full h-full object-cover object-top transition-transform duration-700 ${isOutOfStock ? 'grayscale opacity-70' : ''}`} 
                />
