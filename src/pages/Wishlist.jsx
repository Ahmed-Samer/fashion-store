          ...
          {wishlist.map(p => (
            ...
            <div key={p.id} className="group bg-white dark:bg-slate-800 rounded-2xl border border-slate-100 dark:border-slate-700 overflow-hidden shadow-sm hover:shadow-xl transition duration-500 flex flex-col relative">
              ...
              <div className="p-4 flex-1 flex flex-col">
                <h3 className="font-bold text-slate-800 dark:text-white mb-1 truncate">{p.name}</h3>
                <p className="text-violet-600 dark:text-violet-400 font-bold mb-4">{p.price} EGP</p>
                <button 
                  onClick={() => addToCart(p)} 
                  className="w-full mt-auto bg-slate-900 dark:bg-white text-white dark:text-slate-900 py-2 rounded-xl font-bold text-sm flex items-center justify-center gap-2 hover:bg-slate-800 dark:hover:bg-slate-200 transition"
                >
