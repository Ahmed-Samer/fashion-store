      ...
      <div className="max-w-7xl mx-auto px-4 py-8 md:py-12 animate-fade-in pt-4 md:pt-8">
        ...
        <div className="grid md:grid-cols-2 gap-8 md:gap-12 bg-white/80 dark:bg-slate-800/80 backdrop-blur-xl p-4 md:p-8 rounded-[2rem] md:rounded-[2.5rem] shadow-2xl border border-white/60 dark:border-slate-700 mb-12 relative overflow-hidden transition-colors">
          ...
          <div className="relative w-full h-[500px] bg-white dark:bg-slate-900 rounded-3xl overflow-hidden group border border-slate-100 dark:border-slate-700 shadow-inner">
            <div ref={scrollRef} onScroll={handleScroll} onMouseDown={startDragging} onMouseLeave={stopDragging} onMouseUp={stopDragging} onMouseMove={onDrag} className="absolute inset-0 flex overflow-x-auto snap-x snap-mandatory scrollbar-hide cursor-grab active:cursor-grabbing" style={{scrollBehavior: isDragging ? 'auto' : 'smooth'}}>
                {images.map((img, i) => (
                    <motion.img 
                        ...
                        decoding="async"
                        className={`w-full h-full object-cover flex-shrink-0 snap-center pointer-events-none select-none ${isOutOfStock ? 'grayscale' : ''}`} 
                        alt="" 
                    />
                ))}
            </div>
            ...
            {images.length > 1 && <div className="absolute bottom-4 left-1/2 -translate-x-1/2 flex gap-2 z-10">{images.map((_, i) => (
                <motion.button 
                    key={i} 
                    aria-label={`View image ${i + 1}`}
                    whileTap={{ scale: 0.8 }}
                    onClick={(e) => { e.preventDefault(); if(scrollRef.current) scrollRef.current.scrollTo({ left: i * scrollRef.current.offsetWidth, behavior: 'smooth' }); }} 
                    className={`w-2 h-2 rounded-full transition-all shadow-sm ${activeImg === i ? 'bg-violet-600 w-4' : 'bg-white/80 hover:bg-white'}`}
                ></motion.button>
            ))}</div>}
          </div>
          ...
          <div className="flex flex-col justify-center">
            <div>
              <div className="flex items-center justify-between mb-4">
                  ...
                  <motion.button aria-label="Open size guide" whileTap={{ scale: 0.95 }} onClick={() => setShowSizeGuide(true)} className="flex items-center gap-1 text-xs font-bold text-slate-500 dark:text-slate-400 hover:text-violet-600 dark:hover:text-violet-400 transition border-b border-slate-300 dark:border-slate-600 hover:border-violet-600 pb-0.5">
            ...
            <div className="flex flex-col gap-4 mt-auto">
              <div className="w-full flex items-center justify-between bg-white dark:bg-slate-900 rounded-2xl px-4 py-3 gap-6 border border-slate-200 dark:border-slate-700 shadow-sm">
                  <motion.button aria-label="Decrease quantity" whileTap={{ scale: 0.8 }} onClick={() => qty > 1 && setQty(qty-1)} className="text-slate-400 dark:text-slate-500 hover:text-violet-600 dark:hover:text-violet-400 transition"><Minus size={18}/></motion.button>
                  <span className="font-bold text-2xl text-slate-800 dark:text-white w-6 text-center">{qty}</span>
                  <motion.button aria-label="Increase quantity" whileTap={{ scale: 0.8 }} onClick={() => qty < stock ? setQty(qty+1) : null} className={`text-slate-400 dark:text-slate-500 transition ${qty >= stock ? 'opacity-30 cursor-not-allowed' : 'hover:text-violet-600 dark:hover:text-violet-400'}`}><Plus size={18}/></motion.button>
              </div>
              <div className="flex gap-3">
                  <motion.button whileHover={{ scale: 1.02 }} whileTap={{ scale: 0.95 }} onClick={() => addToCart(selectedProduct, qty)} className="flex-1 bg-slate-900 dark:bg-white text-white dark:text-slate-900 py-4 rounded-2xl font-bold text-sm md:text-lg shadow-lg hover:shadow-xl hover:bg-slate-800 dark:hover:bg-slate-200 transition flex justify-center gap-2 items-center">Add to Cart <ShoppingBag size={18}/></motion.button>
                  <motion.button whileHover={{ scale: 1.02 }} whileTap={{ scale: 0.95 }} onClick={handleBuyNow} className="flex-1 bg-gradient-to-r from-violet-600 to-fuchsia-600 dark:from-violet-500 dark:to-fuchsia-500 text-white py-4 rounded-2xl font-bold text-sm md:text-lg shadow-lg shadow-violet-200 dark:shadow-none hover:shadow-2xl hover:scale-[1.02] transition flex justify-center gap-2 items-center">Buy Now <Rocket size={18}/></motion.button>
              </div>
            </div>
            ) : (
        ...
        <div className="border-t border-slate-200 dark:border-slate-700 pt-12 md:pt-16 mt-12">
            ...
            <div className="grid grid-cols-2 lg:grid-cols-4 gap-4 md:gap-8">
                {related.map(p => (
                    <motion.div whileHover={{ y: -5 }} whileTap={{ scale: 0.98 }} key={p.id} className="group bg-white dark:bg-slate-800 rounded-2xl border border-slate-100 dark:border-slate-700 overflow-hidden cursor-pointer shadow-sm hover:shadow-xl transition-all duration-500 flex flex-col" onClick={() => { navigate(`/product/${p.id}`); window.scrollTo({top: 0, behavior: 'smooth'}); }}>
                        <div className="relative w-full aspect-[3/4] bg-slate-50 dark:bg-slate-900 overflow-hidden">
                            <img loading="lazy" decoding="async" src={p.image} className="w-full h-full object-cover object-top group-hover:scale-110 transition duration-700" alt=""/>
            ...
            <div className="fixed inset-0 z-[100] flex items-center justify-center p-4 bg-black/60 backdrop-blur-sm animate-fade-in">
                <div className="bg-white dark:bg-slate-900 rounded-3xl w-full max-w-lg overflow-hidden shadow-2xl animate-scale-in relative border border-white/10">
                    <div className="flex justify-between items-center p-5 border-b border-slate-100 dark:border-slate-800 bg-slate-50 dark:bg-slate-800/50">
                        ...
                        <motion.button aria-label="Close size guide" whileTap={{ scale: 0.8 }} onClick={() => setShowSizeGuide(false)} className="p-2 hover:bg-slate-200 dark:hover:bg-slate-700 rounded-full transition text-slate-500 dark:text-slate-400"><X size={20}/></motion.button>
                    </div>
                    ...
                    <div className="p-6 overflow-y-auto max-h-[60vh]">
                        ...
                        {sizeChart ? (
                            <div className="overflow-x-auto rounded-xl border border-slate-200 dark:border-slate-700">
                                <table className="w-full text-sm text-left text-slate-600 dark:text-slate-300">
                                    <thead className="text-xs text-slate-700 dark:text-slate-200 uppercase bg-slate-100 dark:bg-slate-800">
                                        <tr>{sizeChart.columns.map((col, i) => <th key={i} className="px-4 py-3">{col}</th>)}</tr>
                                    </thead>
                                    <tbody>
                                        {sizeChart.rows.map((row, i) => (
                                            <tr key={i} className="bg-white dark:bg-slate-900 border-b border-slate-100 dark:border-slate-800 hover:bg-slate-50 dark:hover:bg-slate-800/50">
                                                {row.map((cell, j) => <td key={j} className={`px-4 py-3 ${j === 0 ? 'font-bold text-slate-900 dark:text-white' : ''}`}>{cell}</td>)}
                                            </tr>
                                        ))}
                                    </tbody>
                                </table>
                            ...
                            <div className="text-center py-8 text-slate-500 dark:text-slate-400 bg-slate-50 dark:bg-slate-800/50 rounded-xl border border-dashed border-slate-200 dark:border-slate-700"><p>Standard Size Chart unavailable.</p></div>
                        )}
                        <div className="mt-6 text-xs text-slate-400 text-center">* Measurements are in CM. Allow 1-2cm difference.</div>
                    </div>
                </div>
            </div>
        )}
