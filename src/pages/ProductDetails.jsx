...
import { ArrowRight, Minus, Plus, ShoppingBag, Rocket, Star } from 'lucide-react';

const ProductDetails = ({ products, addToCart }) => {
    const { id } = useParams(); // هات رقم المنتج من الرابط
    ...
    if (selectedProduct) {
        ...
        if (related.length === 0) {
            related = products.filter(p => p.id !== selectedProduct.id).slice(0, 4);
        }
    }

    const images = selectedProduct ? (selectedProduct.images && selectedProduct.images.length > 0 ? selectedProduct.images : [selectedProduct.image]) : [];
    ...
    return (
      <div className="max-w-7xl mx-auto px-4 py-8 md:py-12 animate-fade-in pt-4 md:pt-8">
        ...
        <div className="grid md:grid-cols-2 gap-8 md:gap-12 bg-white/80 backdrop-blur-xl p-4 md:p-8 rounded-[2rem] md:rounded-[2.5rem] shadow-2xl border border-white/60 mb-12 relative overflow-hidden">
          ...
          <div className="relative w-full h-[500px] bg-white rounded-3xl overflow-hidden group border border-slate-100 shadow-inner">
            <div 
                ...
                onMouseMove={onDrag}
                className="absolute inset-0 flex overflow-x-auto snap-x snap-mandatory scrollbar-hide cursor-grab active:cursor-grabbing" 
                style={{scrollBehavior: isDragging ? 'auto' : 'smooth'}}
            >
                {images.map((img, i) => (
                    <img key={i} src={img} id={`prod-img-${i}`} className="w-full h-full object-cover flex-shrink-0 snap-center pointer-events-none select-none" alt="" />
                ))}
            </div>
          ...
          <div className="flex flex-col justify-center">
            <div>
              <div className="flex items-center gap-2 md:gap-3 mb-4 md:mb-6">
                ...
                {selectedProduct.color && <span className="bg-slate-100 text-slate-700 border border-slate-200 text-[10px] md:text-xs px-3 md:px-4 py-1 md:py-1.5 rounded-full font-bold uppercase tracking-wider">{selectedProduct.color}</span>}
            ...
            <div className="flex flex-col gap-4 mt-auto">
              <div className="w-full flex items-center justify-between bg-white rounded-2xl px-4 py-3 gap-6 border border-slate-200 shadow-sm">
                  ...
                  <button onClick={() => setQty(qty+1)} className="text-slate-400 hover:text-violet-600 transition"><Plus size={18}/></button>
              </div>
        ...
        <div className="border-t border-slate-200 pt-12 md:pt-16">
            <h2 className="text-2xl md:text-3xl font-bold mb-8 md:mb-10 text-slate-800">
                {related.length > 0 && products.filter(p => p.category === selectedProduct.category).length > 1 ? 'You Might Also Like' : 'Check These Out'}
            </h2>
                ...
                {related.map(p => (
                    <div key={p.id} className="group bg-white rounded-2xl border border-slate-100 overflow-hidden cursor-pointer shadow-sm hover:shadow-xl transition duration-500 hover:-translate-y-2 flex flex-col" onClick={() => { navigate(`/product/${p.id}`); window.scrollTo({top: 0, behavior: 'smooth'}); }}>
                        ...
                        <div className="p-3 flex flex-col gap-1 bg-white">
                            ...
                            <p className="text-violet-600 font-bold text-sm">{p.price} EGP</p>
                        </div>
                    </div>
                ))}
            </div>
