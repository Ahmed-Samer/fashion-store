...
const AdminDashboard = ({ user, products, orders, showNotification }) => {
  ...
  const handleSavePromo = async (e) => { e.preventDefault(); try { await addDoc(collection(db, 'artifacts', appId, 'public', 'data', 'promo_codes'), { code: promoForm.code.toUpperCase().trim(), discount: Number(promoForm.discount), createdAt: new Date().toISOString() }); setPromoForm({ code: '', discount: '' }); showNotification('Promo Created 🎟️'); } catch(e){} };
        ...
        <div className="grid lg:grid-cols-3 gap-8">
          <div className="lg:col-span-1 bg-white/80 backdrop-blur-md p-6 md:p-8 rounded-3xl border border-white/60 shadow-lg h-fit sticky top-4">
            ...
            <form onSubmit={handleSaveProduct} className="space-y-4">
              ...
              <div className="space-y-2">
                <div className="flex justify-between items-center"><label className="text-xs font-bold text-slate-500 uppercase">Tags</label><button type="button" onClick={handleGenerateTags} disabled={isGeneratingTags} className="text-xs text-violet-600 bg-violet-50 px-2 py-1 rounded-md font-bold">{isGeneratingTags ? '...' : 'Auto Tags ✨'}</button></div>
      ...
      {activeTab === 'orders' && (
        <div className="space-y-6">
            <div className="bg-white/80 p-4 rounded-2xl shadow-sm border border-slate-100 flex items-center gap-3">
                ...
                <input type="text" placeholder="ابحث برقم الأوردر (Order ID) أو رقم التليفون..." className="w-full bg-transparent outline-none text-slate-700 font-bold" value={searchTerm} onChange={(e) => setSearchTerm(e.target.value)} />
            </div>
            <div className="space-y-4">
            {filteredOrders.length === 0 ? <div className="text-center py-10 text-slate-400">لا توجد أوردرات مطابقة للبحث.</div> :
